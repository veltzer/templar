...
def keys_read():
    ...
    for line in output.split('\n'):
        if not line.startswith('pub'):
            continue
        inner_parts = line.split()
        all_keys.add(inner_parts[1].split('/')[1])


def keys_have(key_id_to_check):
    return key_id_to_check in all_keys


def subprocess_wrap(subprocess_args, silent=opt_silent, check=opt_check):
    if silent:
        if check:
