...
'Topic :: Utilities' ],
	data_files=[
		...
		('/usr/bin', ['bin/templar_cmd']),
		('/usr/bin', ['bin/make_helper']),
		('/usr/bin', ['bin/release_helper']),
		('/usr/bin', ['bin/wrapper_css_validator']),
		('/usr/bin', ['bin/wrapper_noerr']),
		('/usr/bin', ['bin/wrapper_ok']),
		('/usr/bin', ['bin/wrapper_silent']),
	],
)
